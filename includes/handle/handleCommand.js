...
const dataAdbox = require('./../../modules/commands/cache/data.json');
   ...
   if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true && !NDH.includes(senderID) && !findd && event.isGroup == true) return api.sendMessage('Hiện tại đang kích hoạt chế độ Quản trị viên, chỉ Quản trị viên được sử dụng Bot', event.threadID, event.messageID)
    ...
    if (usages[senderID].usages <= 0 && !["nhanluot","check","luotdung", "daily"].includes(commandName)) return api.sendMessage("Bạn hiện đã hết lượt sử dụng Bot\nDùng lệnh #nhanluot để nhận thêm 100 lượt dùng\nDùng lệnh luotdung để xem thêm các tùy chọn về lượt dùng Bot", threadID, messageID);
    if (!command) {
      var allCommandName = [];
